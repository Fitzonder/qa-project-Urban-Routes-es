...
class UrbanRoutesPage:
    ...
    def set_route(self, address_from, address_to):
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(UrbanRoutesPage.from_field))
        self.driver.find_element(*self.from_field).send_keys(address_from)
        WebDriverWait(self.driver, 10).until(EC.text_to_be_present_in_element_value(UrbanRoutesPage.from_field, address_from))
        self.driver.find_element(*self.to_field).send_keys(address_to)
        WebDriverWait(self.driver, 10).until(EC.text_to_be_present_in_element_value(UrbanRoutesPage.to_field, address_to))
    ...
    def ask_taxi_option(self):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(UrbanRoutesPage.ask_taxi_button))
        return self.driver.find_element(*self.ask_taxi_button).click()

    #Seleccionar tarifa comfort
    def select_comfort_tariff(self):
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(UrbanRoutesPage.comfort_button_title))
        self.driver.find_element(*self.comfort_button_title).click()

    #Agregar número de teléfono
    def fill_phone_number(self, phone_number):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(UrbanRoutesPage.number_field_module))
    ...
    def confirm_phone_code(self):
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(UrbanRoutesPage.phone_code_field))
    ...
    def add_credit_card(self, card_number, card_code):
        WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable(UrbanRoutesPage.payment_field))
        ...
        WebDriverWait(self.driver, 10).until(
            EC.visibility_of_element_located(UrbanRoutesPage.payment_choice))
    ...
    def write_message_for_driver(self, driver_comment):
        WebDriverWait(self.driver, 10).until(
            EC.visibility_of_element_located(UrbanRoutesPage.driver_message_field))
    ...
    def request_blanket(self):
        WebDriverWait(self.driver, 10).until(
            EC.visibility_of_element_located(UrbanRoutesPage.mantas_slider))
    ...
    def request_ice_creams(self):
        ...
        close_buttons = self.driver.find_elements(*self.close_window_button)
        if close_buttons:
            close_button = next(
                (el for el in close_buttons if el.is_displayed() and el.is_enabled()),
                None
            )
            if close_button:
                try:
        ...
        WebDriverWait(self.driver, 10).until(EC.invisibility_of_element_located(overlay))
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(UrbanRoutesPage.ice_cream_counter))
        counter = self.driver.find_element(*self.ice_cream_counter)
        self.driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", counter)
        try:
            counter.click()
            counter.click()
        except Exception:
            self.driver.execute_script("arguments[0].click();", counter)
            self.driver.execute_script("arguments[0].click();", counter)

    def get_ice_cream_count(self):
        WebDriverWait(self.driver, 10).until(EC.text_to_be_present_in_element(UrbanRoutesPage.ice_cream_chosen, "2"))
   ...
   #Regresará "true" si el botón de pedido de taxí está disponible
    def is_send_request_button_displayed(self):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(UrbanRoutesPage.send_taxi_request))
        return self.driver.find_element(*self.send_taxi_request).is_displayed()

    #Termina la tarea pidiendo finalmente el taxi
    def request_taxi(self):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(UrbanRoutesPage.send_taxi_request))
        request_button = self.driver.find_element(*self.send_taxi_request)
        self.driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", request_button)
        try:
            request_button.click()
        except Exception:
            self.driver.execute_script("arguments[0].click();", request_button)
